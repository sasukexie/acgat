...
def save_split_dataloaders(config, dataloaders):
    ...
    for dataloader in dataloaders:
        if isinstance(dataloader, KnowledgeBasedDataLoader):
            gs = []
            for dl in [dataloader.general_dataloader, dataloader.kg_dataloader]:
                generator_state = dl.generator.get_state()
                dl.generator = None
                dl.sampler.generator = None
                gs.append(generator_state)
            Serialization_dataloaders += [(dataloader, gs)]
        else:
...
def load_split_dataloaders(config):
    ...
    with open(dataloaders_save_path, "rb") as f:
        dataloaders = []
        for dataloader, generator_state in pickle.load(f):
            if isinstance(dataloader, KnowledgeBasedDataLoader):
                gs = generator_state
                for dl, generator_state in zip([dataloader.general_dataloader, dataloader.kg_dataloader], gs):
                    generator = torch.Generator()
                    generator.set_state(generator_state)
                    dl.generator = generator
                    dl.sampler.generator = generator
